...
export class GroupComponent extends AbstractComponent implements OnInit, RightKindInterface {
  ...
  // 선택 전체 조직 데이터
  public selectAllOrgData: Array<string> = [];
  ...
  ngOnInit() {
    ...
    if (this.menuDetailData.fltrVal) {
      for (let i = 0;  i < this.menuDetailData.fltrVal.length; i++) {
        this.selectAllOrgData.push(this.menuDetailData.fltrVal[i].code[0]);
        let addrNameData = this.menuDetailData.fltrVal[i].name;
        let addrCodeData = Utils.ArrayUtil.remove(this.menuDetailData.fltrVal[i].code, 0);
  ...
  public getFilterData(): any {
    ...
    for (let i = 0;  i < this.selectAllDepthCodeData.length; i++) {
      ...
      // 선택 데이터에 조직코드 추가
      filterDepthData.push(this.selectAllOrgData[i]);
      filterDepthData = filterDepthData.concat(this.selectAllDepthCodeData[i]);

      // 주소 데이터 셋팅
      fltrVal.push({
        code: filterDepthData,
  ...
  public removeAddress(i: number): void {
    ...
    this.selectAllOrgData = Utils.ArrayUtil.remove(this.selectAllOrgData, i);
  ...
  public checkSelectDepthValid(): void {
    ...
    this.selectAllOrgData.push(this.selectVal);
