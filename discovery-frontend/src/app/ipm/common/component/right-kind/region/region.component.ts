...
export class RegionComponent extends AbstractComponent implements OnInit, RightKindInterface {
  ...
  public clickLastDepth(item, depth): void {
    ...
    if (!item.hierarchy[depth - 1]) {
      item.hierarchy[depth - 1] = item.code;
    }
    item.fltrUid = this.menuDetailData.fltrUid;
    item.hierarchy = _.cloneDeep(item.hierarchy.slice(0, depth));
